...
option_list = list(
    make_option(c("-fA", "--fileA"), type="character", default=NULL, 
              help="dataset file name", metavar="character"),

    make_option(c("-fB", "--fileB"), type="character", default=NULL, 
...
if (is.null(opt$fileA) || is.null(opt$out) || is.null(opt$fileB)){
...
data_table      <- read.table(paste(res_folder, opt$fileA, sep=""), comment.char = "#", col.names = columnsA)
data_table_cmp  <- read.table(paste(res_folder, opt$fileB, sep=""), comment.char = "#", col.names = columnsB)
...

{
    ...
    title(main=expression("Absolute difference in efficiency: e"[absDiff]*" = e(G"[newWeighted]*") - e(G"[new]*")"))
