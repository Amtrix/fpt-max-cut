...
func_localize() {
  ...
  #  allowed_total_time_seconds=2000
    allowed_total_time_seconds=172800
   # 21600=6*60*60 = 6 hours

 # allowed_total_time_seconds=5

    mkdir -p $experiment_outdir/solvers/real-world-small-hard/
    $builddir/./$selected_build -action "kernelization" -iterations 1 -disk-suite real-world-small-hard -total-allowed-solver-time $allowed_total_time_seconds \
                    -do-signed-reduction \
                    -support-weighted-result \
                    -exact-early-stop-v 700 \
                    -number-of-threads 1 \
                    -locsearch-iterations 100 \
                    -benchmark-output $experiment_outdir/solvers/real-world-small-hard/out > $experiment_outdir/solvers/real-world-small-hard/out-exe
