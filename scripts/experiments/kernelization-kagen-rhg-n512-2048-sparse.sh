...
do
    for j in "${arrrhg[@]}"
    do
        ./$selected_build -action "kernelization" -iterations $num_iterations -sample-kagen $kagen_instances -num-nodes $i -num-edges-lo 0 -num-edges-hi $((i*8)) -rhgfix $j -total-allowed-solver-time -1 \
        ...
        ./$selected_build -action "kernelization" -iterations $num_iterations -sample-kagen $kagen_instances -num-nodes $i -num-edges-lo 0 -num-edges-hi $((i*8)) -rhgfix $j -total-allowed-solver-time -1 -support-weighted-result \
